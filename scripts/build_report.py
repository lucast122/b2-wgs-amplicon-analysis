...
<p class="muted">Planned dissemination (see <code>SHIPPING_PLAN.md</code>). Raw reads: NCBI SRA
SRR24887495–SRR24888648. <b>Metagenome-assembled genomes</b> (genome-resolved, this study) →
ENA/NCBI BioProject, MIMAG-compliant (CheckM2 quality + GTDB-Tk taxonomy) — the primary reusable
deposit, given this enclosed, largely uncultured tropical-rainforest soil. <b>Custom AMF/EMF Kaiju
database</b> (detects mycorrhizal fungi from bulk shotgun, which SSU/sylph/standard tools miss) +
classifier → Zenodo (DOI). <b>Code</b> → github.com/lucast122/b2-wgs-amplicon-analysis (Zenodo
release DOI). Taxonomic + functional abundance matrices and the AMF/EMF workbooks → Supplementary
Data. A machine-readable <code>samples.tsv</code> (accession ↔ condition/site/plot/timepoint) keys
the reads for reuse.</p>
